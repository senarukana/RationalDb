...
func TestCRAMMD5GetChallenge(t *testing.T) {
	...
	if m, _ := regexp.MatchString(challengePattern, challenge); !m {
		t.Error("The challenge %s doesn't look like an RFC822 msg-id (%s).", challengePattern, challenge)
	}
}
...
func TestCRAMMD5GetExpected(t *testing.T) {
	...
	if !strings.HasPrefix(expected, "ala ") {
		t.Error("The proof doesn't start with the username: %s.", expected)
	}

	if length := len(expected); length != 32+len("ala ") {
		t.Error("%s doesn't look like a correct CRAM-MD5 digest (the length is not 32).", expected)
	}
}
