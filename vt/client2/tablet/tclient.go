...
import (
	...
	tproto "github.com/senarukana/rationaldb/vt/tabletserver/proto"
)
...
type StreamResult struct {
	call    *rpcplus.Call
	sr      chan *eproto.QueryResult
	columns *eproto.QueryResult
	// current result and index on it
	qr    *eproto.QueryResult
	index int
	err   error
}
...
func (conn *Conn) Exec(query string, bindVars map[string]interface{}) (db.Result, error) {
	...
	if conn.stream {
		sr := make(chan *eproto.QueryResult, 10)
...
func NewResult(rowCount, rowsAffected, insertId int64, fields []eproto.Field) *Result {
	return &Result{
		qr: &eproto.QueryResult{
...
func (result *Result) Fields() []eproto.Field {
