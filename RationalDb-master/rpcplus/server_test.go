...
func listenTCP() (net.Listener, string) {
	...
	if e != nil {
		log.Critical("net.Listen tcp :0: %v", e)
	}
...
func testRPC(t *testing.T, addr string) {
	...
	if err != nil {
		t.Error("Add: expected no error but got string %q", err.Error())
	}
	if reply.C != args.A+args.B {
		t.Error("Add: expected %d got %d", reply.C, args.A+args.B)
	}
	...
	if err == nil {
		...
		t.Error("BadOperation: expected can't find method error; got %q", err)
	}
	...
	if addCall.Error != nil {
		t.Error("Add: expected no error but got string %q", addCall.Error.Error())
	}
	if addReply.C != args.A+args.B {
		t.Error("Add: expected %d got %d", addReply.C, args.A+args.B)
	}

	mulCall = <-mulCall.Done
	if mulCall.Error != nil {
		t.Error("Mul: expected no error but got string %q", mulCall.Error.Error())
	}
	if mulReply.C != args.A*args.B {
		t.Error("Mul: expected %d got %d", mulReply.C, args.A*args.B)
	}
	...
	if err != nil {
		t.Error("Scan: expected no error but got string %q", err.Error())
	} else if reply.C != Val {
		t.Error("Scan: expected %d got %d", Val, reply.C)
	}
	...
	if err != nil {
		t.Error("String: expected no error but got string %q", err.Error())
	}
	expect := fmt.Sprintf("%d+%d=%d", args.A, args.B, args.A+args.B)
	if str != expect {
		t.Error("String: expected %s got %s", expect, str)
	}
	...
	if err != nil {
		t.Error("Mul: expected no error but got string %q", err.Error())
	}
	if reply.C != args.A*args.B {
		t.Error("Mul: expected %d got %d", reply.C, args.A*args.B)
	}
	...
	if err != nil {
		t.Error("TakesContext: expected no error but got string %q", err.Error())
	}
}
...
func testServeRequest(t *testing.T, server *Server) {
	...
	if err == nil {
		t.Error("expected error calling Arith.Add with nil arg")
	}
}
