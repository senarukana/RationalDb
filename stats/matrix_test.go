...
func TestMatrix(t *testing.T) {
	...
	if !strings.Contains(m.String(), want1) {
		t.Error("want containing %s, got %s", want1, m.String())
	}
	if !strings.Contains(m.String(), want2) {
		t.Error("want containing %s, got %s", want2, m.String())
	}
	if !strings.Contains(m.String(), want3) {
		t.Error("want containing %s, got %s", want3, m.String())
	}
	counts := m.Data()
	if counts["m1"]["n1"] != 1 {
		t.Error("want 1, got %d", counts["m1"]["n1"])
	}
	if counts["m1"]["n2"] != 1 {
		t.Error("want 1, got %d", counts["m1"]["n2"])
	}
	if counts["m2"]["n1"] != 2 {
		t.Error("want 2, got %d", counts["m2"]["n1"])
	}
	...
	if !strings.Contains(mf.String(), want1) {
		t.Error("want containing %s, got %s", want1, mf.String())
	}
	if !strings.Contains(mf.String(), want2) {
		t.Error("want containing %s, got %s", want2, mf.String())
	}
	if !strings.Contains(mf.String(), want3) {
		t.Error("want containing %s, got %s", want3, mf.String())
	}
}
...
func TestMatrixHook(t *testing.T) {
	...
	if gotname != "matrix2" {
		t.Error("want matrix2, got %s", gotname)
	}
	if gotv != v {
